...
// Inside Search.svelte
let { title } = $props();
let searched = $state([]);
let loading = $state(true);

// This runs whenever 'title' changes
$effect(() => {
  if (title) {
    fetchResults(title);
  }
...
async function fetchResults(searchQuery) {
  loading = true;
  try {
    const searchresult = await api.manwhas.getSearch(searchQuery);
  ...
  {:else if !loading}
    ...
    <section class="section">
      <div class="container">
        <div class="section-header">
          <h2>Search</h2>
          <span class="manwha-count">{searched.length} titles</span>
        </div>
        <div class="grid">
